...
int FindAmountOfNotEmptyFiles(FileData* files, const int& from, const int& to) {
    ...
    for (size_t i = 0; i < to; ++i)
    {
        if (files[i].eof)
        {
            ...
            while (files[swap_index].eof && swap_index > i)
            {
                --swap_index;
            }
            if (swap_index > i)
            {
                // swap(files[i], files[swap_index])
                string Ifilename = files[i].filename;
                /*bool Ieof = files[i].eof;
                bool Ieos = files[i].eos;*/
                MyVeryGoodPair Ielem(files[i].elem);

                files[i].filename = files[swap_index].filename;
                files[i].eof = files[swap_index].eof;
                files[i].eos = files[swap_index].eos;
                files[i].elem.RemakeMyVeryGoodPair(files[swap_index].elem.GetNumber(), files[swap_index].elem.GetNumber());

                files[swap_index].filename = Ifilename;
                files[swap_index].eof = false;
                files[swap_index].eos = false;
                files[swap_index].elem.RemakeMyVeryGoodPair(Ielem);

                files[i].infile.close();
                files[i].infile.open(files[i].filename, ifstream::binary);
                auto pos = files[swap_index].infile.tellg();
                files[i].infile.seekg(pos, files[i].infile.beg);

                files[swap_index].infile.close();
                files[swap_index].infile.open(files[swap_index].filename, ifstream::binary);
                files[swap_index].infile.seekg(0, files[swap_index].infile.end);
                //---------------//
                ++amount_of_not_empty_files;
            }
        }
