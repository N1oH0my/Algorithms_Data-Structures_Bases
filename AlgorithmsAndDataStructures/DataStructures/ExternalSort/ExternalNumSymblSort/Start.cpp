...
//-----------------------------------FOR-ELEMENTS-----------------------------------------------------------------//
void ReadNext(FileData* file) {
    int prevN = file->GetPair().GetNumber();
    char prevS = file->GetPair().GetSymbol();
    if (file->InFilePeek() != EOF)
    {
        ...
        if (prevN > file->GetPair().GetNumber() || (prevN == file->GetPair().GetNumber() && prevS > symbol))
        {
            file->SetEos(true);
        }
    }
...
FileData& FindMinFileInSequences(FileData** files, const int& from, const int& to) {
    ...
    while (i < to)
    {
        int num1 = files[i]->GetPair().GetNumber();
        int num2 = fd->GetPair().GetNumber();
        if (!files[i]->GetEos() && ((num1<num2) || (num1==num2 && files[i]->GetPair().GetSymbol() < fd->GetPair().GetSymbol())))
        {
            fd = files[i];
        }
